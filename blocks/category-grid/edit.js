( function ( wp, bw ) {
	...
	registerBlockType( 'bw/category-grid', {
		edit: function ( props ) {
			var attributes = props.attributes;
			var setAttributes = props.setAttributes;
			var blockProps = useBlockProps( { className: 'bw-block bw-category-grid' } );
			...
			return el(
				...
				el(
					...
					el( 'div', { className: 'bw-block__inner' },
						el( RichText, {
							tagName: 'h2',
							className: 'bw-category-grid__heading',
							placeholder: __( 'Überschrift (optional) …', 'bodywings' ),
							value: attributes.heading,
							onChange: function ( v ) { setAttributes( { heading: v } ); },
							allowedFormats: [],
						} ),
						el( 'p', { className: 'bw-block__editor-note', style: { opacity: 0.6, fontStyle: 'italic' } }, __( 'Die tatsächlichen Kategorien erscheinen im Frontend.', 'bodywings' ) )
					)
				)
			);
		},
